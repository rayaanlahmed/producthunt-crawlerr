  ...
  for (let idx = 0; idx < pages.length; idx++) {
    ...
    if (page.metadata?.sourceURL?.includes('/products/')) {
      ...
      const product = {
        ...
        founder: await extractFounder(page),
        founderLinkedIn: await extractFounderLinkedIn(page),
        founderAppSumoProfile: await extractFounderAppSumoProfile(page),
...
function extractPricingTiers(page) {
  ...
  if (pricing.tiers.length === 0 && html) {
    ...
    for (let i = 1; i < paymentSections.length; i++) {
      ...
      // Extract tier name from before section
      const tierNameMatch = beforeSection.match(/<p[^>]*class="[^"]*text-xl[^"]*"[^>]*>([^<]+)<\/p>\s*$/);
      const tierName = tierNameMatch ? tierNameMatch[1].trim() : null;
      ...
      if (price) {
        const tierNumMatch = tierName ? tierName.match(/(?:Tier|Plan|Code)\s*(\d+)/i) : null;
...
async function extractFounderInfo(page) {
  ...
  if (appSumoMatch) {
    ...
    // Only use this if it looks like a real name (not a review snippet)
    const rejectWords = ['the', 'from', 'founder', 'are', 'is', 'was', 'beyond', 'genius',
                        'great', 'amazing', 'best', 'team', 'company', 'recognized', 'that'];
  ...
  const explicitPatterns = [
    /(?:Co-?founder|Founder)[:\s]+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/i,       // "Founder: Name" or "Co-founder Name"
    /(?:Founded by|Created by)[:\s]+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/i,     // "Founded by Name"
    /([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)[,\s]+(?:Co-?founder|Founder)/i,       // "Name, Founder"
    /(?:CEO|Co-CEO)[:\s]+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/i,                // "CEO: Name"
  ];

  for (const pattern of explicitPatterns) {
    const match = content.match(pattern);
    if (match && match[1] && match[1].split(/\s+/).length >= 2) {  // Must have first + last name
      const linkedInUrl = content.match(/(https?:\/\/(?:www\.)?linkedin\.com\/in\/[a-zA-Z0-9-]+)/);
 ...
 */
async function crawlAppSumoProgressive(categories = [], maxProducts = 50, sortBy = null, callbacks = {}) {
