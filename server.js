...
app.post('/api/crawl', async (req, res) => {
    try {
        const { categories = [], maxPages = 30, sortBy = null, crawlMode = 'fast' } = req.body;

        console.log(`Received crawl request - Categories: ${categories.join(', ') || 'All'}, Max Pages: ${maxPages}${sortBy ? `, Sort: ${sortBy}` : ''}, Mode: ${crawlMode}`);
        ...
        if (!validModes.includes(crawlMode)) {
            return res.status(400).json({ error: `crawlMode must be one of: ${validModes.join(', ')}` });
        }
        ...
        await crawlAppSumoProgressive(categories, maxPages, sortBy, crawlMode, {
...
// Recrawl endpoint for selected products in complete mode
app.post('/api/recrawl', async (req, res) => {
    try {
        const { urls } = req.body;
        ...
        console.log(`\nRecrawling ${urls.length} products in Complete Mode...`);

        // Import Firecrawl and processProducts
        ...
        for (let i = 0; i < urls.length; i++) {
            const url = urls[i];
            console.log(`  Recrawling ${i + 1}/${urls.length}: ${url}`);

            try {
                const page = await firecrawl.scrapeUrl(url, {
                    formats: ['markdown', 'html'],
                    onlyMainContent: false,  // Complete mode - get all content
                    waitFor: 1000,
                });
        ...
        console.log(`Recrawl complete: ${products.length} products updated`);
        ...
        console.error('Error in /api/recrawl:', error);
        res.status(500).json({
            error: error.message || 'An error occurred while recrawling'
        });
    }
});
